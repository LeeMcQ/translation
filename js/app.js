  ...
  shell(`
    ...
    <section class="grid3">
      ...
      <article class="card">
        ...
        <p>Chrome speech is free. Liturgy, Kaaps slang, and the SDA glossary never touch an API. Machine translation is only leftover sentences.</p>
      </article>
    </section>
  `);
}
...
function allTerms() {
  return [
    ...GLOSSARY.map((r) => ({ ...r, cat: r.cat || "Doctrine" })),
    ...SLANG.map((r) => ({ af: r.af, en: r.en, note: r.note || "", cat: r.cat || "Kaaps / slang" })),
  ];
}
...
function renderGlossary() {
  ...
  shell(`
    <section class="prose">
      <h1>Pulpit glossary</h1>
      <p>${terms.length} pinned terms — Adventist doctrine plus Cape / Kaaps slang. Speech errors like “seel” are corrected to “seël” before translation.</p>
      <input id="gq" class="input" placeholder="Search Sabbath, lekker, seël…" />
      <div class="chips" id="gchips"></div>
      <div id="glist" class="terms"></div>
    </section>
  `);
  let cat = "All";
  const cats = ["All", "Doctrine", "Kaaps / slang"];
  const paintChips = () => {
    document.getElementById("gchips").innerHTML = cats
      .map(
        (c) =>
          `<button type="button" class="chip ${c === cat ? "on" : ""}" data-cat="${esc(c)}">${esc(c)}</button>`,
      )
      .join("");
  };
  const paint = () => {
    const q = document.getElementById("gq").value;
    let rows = searchGlossary(q, terms);
    if (cat !== "All") rows = rows.filter((r) => r.cat === cat);
    document.getElementById("glist").innerHTML = rows
      .map(
        (r) =>
          `<div class="term"><b>${esc(r.af)}</b><span>${esc(r.en)}</span>${r.note ? `<i>${esc(r.note)}</i>` : ""}${r.cat ? `<i>${esc(r.cat)}</i>` : ""}</div>`,
      )
      .join("");
  };
  paintChips();
...
function renderHost() {
  if (!state.host) {
    ...
    state.host.bus.setSnapshot(() => ({ type: "sync", lines: state.host.lines }));
